...
<div class="edit-toolbar">
  ...
  <div class="edit-extra">
    ...
    <button id="downloadPptxBtn" type="button">Download PPTX</button>
...
<main class="slides" id="slides">
  ...
  <section class="slide"><div class="content">
    <div class="top"><div class="logo"><img src="assets/edra-logo.png" alt="EDRA"></div><div>11 · Ranking by Period</div></div>
    <div><h1>EDRA Ranking by Year / Period</h1><p class="sub">No combined ranking is used. 2025 and 5M2026 are calculated separately, with rank and market share shown for each product group.</p></div>
    <div class="card glow-card"><h2>Separate Rank & Market Share</h2><div id="rankSummaryCards" style="margin-top:22px"></div></div>
    <div class="foot"><span>Ranking source: 2025 full-year and 5M2026 brand import data</span><span>11</span></div>
  </div></section>
...
function rankSummary() {{
  ...
  const tableBlock = (year,color) => `<div style="border:1px solid var(--line);border-radius:24px;padding:20px;background:#0b0b10;min-height:390px">
    <div style="display:flex;align-items:end;justify-content:space-between">
      <div><div class="label">Period</div><h2 style="color:${{color}};font-size:34px">${{periodLabel(year)}}</h2></div>
      <div class="note">Rank and market share calculated inside this period only</div>
    </div>
    <table style="margin-top:18px;font-size:14px">
      <thead><tr><th>Product</th><th>Rank</th><th>EDRA Qty</th><th>Share</th></tr></thead>
      <tbody>
        ${{cats.map(cat=>{{ const r=rankMarket(rankBlock(cat,year)); return `<tr><td>${{cat}}</td><td style="color:${{color}};font-weight:950">#${{r.edra.rank||'-'}}</td><td>${{fmt(r.edra.quantity||0)}}</td><td class="pos">${{sharePct(r.share)}}</td></tr>`; }}).join('')}}
      </tbody>
    </table>
  </div>`;
  el('rankSummaryCards').innerHTML = `<div style="display:grid;grid-template-columns:1fr 1fr;gap:18px">${{tableBlock(2025,'var(--orange)')}}${{tableBlock(2026,'var(--purple)')}}</div>`;
  el('rankSummaryCards').innerHTML += `<div style="margin-top:16px;border:1px solid var(--line);border-radius:18px;padding:12px 16px;color:var(--muted);font-size:14px">This slide intentionally removes combined ranking. Detailed category slides continue with separate 2025 and 5M2026 charts.</div>`;
}}
...
function editableTextNodes() {{
  return [...document.querySelectorAll('.slide h1,.slide h2,.slide .sub,.slide .note,.slide .label,.slide .foot span,.slide .mini-metric b,.slide .product-preview-item b,.slide .editable-copy')];
}}
...
function saveEdits(showAlert=true) {{
  ...
  try {{
    ...
    try {{
      localStorage.setItem(EDIT_KEY, JSON.stringify({{texts: editStore.texts, images: {{}}}}));
    }} catch(err) {{}}
    const msg = 'Ảnh nhiều hoặc quá nặng nên trình duyệt không lưu tự động được hết. Text đã được ưu tiên lưu; ảnh vẫn còn trong phiên hiện tại. Hãy bấm Download HTML hoặc Export trước khi reload trang.';
...
async function downloadPptx() {{
  ...
  try {{
    ...
    if(!window.html2canvas || !PptxGen) throw new Error('PPTX export libraries are not ready.');
    const wasPresenting = document.body.classList.contains('presenting');
    const wasEditing = editMode;
    if(wasEditing) setEditMode(false);
    document.body.classList.add('presenting','exporting');
    const pptx = new PptxGen();
    pptx.layout = 'LAYOUT_WIDE';
    pptx.author = 'EDRA';
    pptx.subject = 'EDRA Import and Sell-out Dashboard';
    pptx.title = 'EDRA Import and Sell-out Performance';
    const originalIndex = current;
    const slides = [...document.querySelectorAll('.slide')];
    const deck = el('slides');
    const oldTransition = deck.style.transition;
    deck.style.transition = 'none';
    for(let i=0;i<slides.length;i++) {{
      if(btn) btn.textContent = `PPTX ${{i+1}}/${{slides.length}}`;
      goTo(i);
      ...
      s.background = {{ color: '050505' }};
      s.addImage({{ data: img, x: 0, y: 0, w: 13.333, h: 7.5 }});
    }}
    goTo(originalIndex);
    deck.style.transition = oldTransition;
    if(!wasPresenting) document.body.classList.remove('presenting');
    document.body.classList.remove('exporting');
    if(wasEditing) setEditMode(true);
    await pptx.writeFile({{ fileName: 'edra-dashboard-slides.pptx' }});
  }} catch(err) {{
    alert('Chưa xuất được PPTX. Hãy kiểm tra mạng rồi thử lại, vì nút này cần tải thư viện xuất PPTX lần đầu. ' + (err && err.message ? err.message : ''));
  }} finally {{
    document.body.classList.remove('exporting');
    if(btn) {{ btn.textContent = oldText || 'Download PPTX'; btn.disabled = false; }}
  }}
}}
...
function bindEditorButtons() {{
  ...
  bind('downloadPptxBtn', downloadPptx);
